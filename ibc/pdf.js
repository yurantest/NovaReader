...
export const makePDF = async file => {
    ...
    book.getText = async (index) => {
        try {
            const page = await pdf.getPage(index + 1)
            const textContent = await page.getTextContent()
            return textContent.items.map(item => item.str).join(' ')
        } catch(e) {
            console.error('PDF getText error:', e)
            return ''
        }
    }
    ...
    book.getCover = async () => {
        try {
            ...
            console.log('[PDF] Getting cover, page:', page.pageNumber)
            const blob = await renderPage(page, true)
            console.log('[PDF] Cover blob:', blob)
            return blob
        } catch (e) {
            console.error('[PDF] getCover error:', e)
            return null
        }
    }
